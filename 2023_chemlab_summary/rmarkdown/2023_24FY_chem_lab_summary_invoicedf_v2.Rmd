...
FY_data <- read_csv(here("data", "2023_24FY_invoice_df.csv")) %>% 
  clean_names() %>% 
  mutate(test = tolower(test)) %>% 
  mutate(date_collected = mdy(date_collected))

FY_data <- FY_data %>% 
  mutate(price = as.numeric(FY_data$price))

FY_tests <- unique(FY_data$test)

FY_chem_data <- FY_data %>% 
  ...
  mutate(test = recode(test, "alk" = "alkalinity",
                       "chla" = "chlorophyll-a",
                       "cond" = "conductivity",
...
FY_chem_tests <- unique(FY_chem_data$test)

Cal_data <- read_csv(here("data", "2023_chemlab_costs.csv")) %>% 
  clean_names() %>% 
  mutate(date_collected = mdy(date_collected)) 

Cal_data <- Cal_data %>% 
  mutate(price = as.numeric(Cal_data$price))

Cal_chem_data <- Cal_data %>% 
  mutate(test = tolower(test)) %>% 
  filter(test %in% c("absorbance scan", "alk", "anions", "chla", "cations", "cond", "dic", "doc", "do", "filtration", "nh3", "no2", "no3", "part c/n", "part fe", "part p", "ph", "phycocyanin", "srsi", "tdn", "tdp", "turbidity")) %>% 
  mutate(test = recode(test, "alk" = "alkalinity",
                       "chla" = "chlorophyll-a",
...
Cal_chem_tests <- unique(Cal_chem_data$test)
...
total_sm <- length(unique(FY_data$sample_id))

# Total number of samples analysed by the Chemistry Lab df
total_chem_count <- length(unique(FY_chem_data$sample_id))
...
test_summary <- as.data.frame(table(FY_chem_data$test))
...
total_external_costs <- Cal_chem_data %>% 
...
customers <- (unique(Cal_chem_data$customer_id))

external_customers <- (unique(total_external_costs$customer_id))

total_chem_costs_count <- sum(Cal_chem_data$price, na.rm = TRUE)

project_costs <- Cal_chem_data %>% 
...
# Manually changing REMEdiate cost recovery from $16,694.00 to $16,667.44 since the former is what was invoiced. 
external_project_costs$total[external_project_costs$customer_id == "REMEdiate"] <- 16667.44

total_external_costs_count<- round(sum(external_project_costs$total, na.rm = TRUE), digits = 2) %>% 
...
The total number of samples processed by the IISD-ELA Analytical Service Lab (IISD-ASL) in the 2023-2024 fiscal year was `r total_chem_count`. The following plot displays how many samples were analysed for each test in the 2023-2024 fiscal year.
