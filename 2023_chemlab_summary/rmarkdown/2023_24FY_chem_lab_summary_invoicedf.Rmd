...
# Dataframe that includes all columns, but is missing records (rows)
data <- read_csv(here("data", "2023_24FY_chemlab_data.csv")) %>% 
  clean_names() %>% 
  rename(sample_id = sample_number) %>% 
...
# Creating new df that only includes needed columns
test_data <- data %>% 
  select("sample_id", "test", "test_price", "customer_test_price", "customer_id") %>% 
  distinct()

tests <- unique(data$test)

# Dataframe that includes all records (there are missing DOC records in the data df), but does not have a "customer_id" column. Need to create new "customer_id" column from records in test_data df based on "sample_id" and "test" as primary keys. There will be records in cost_data df that do not have records in test_data df (i.e. there are missing DOC records in test_data)
costs_data <- read_csv(here("data", "2023_24FY_invoice_df.csv")) %>% 
  clean_names()# %>% 

merged_data <- left_join(costs_data, test_data, by = c("sample_id", "test"))

length(costs_data$sample_id)
length(merged_data$sample_id)
length(test_data$sample_id)
...
# Note: the chunks below need to be updated with new dfs; they are based on old df's.
