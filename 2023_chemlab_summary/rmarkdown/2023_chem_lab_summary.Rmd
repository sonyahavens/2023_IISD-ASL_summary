...
data <- read_csv(here("data", "2023_chemlab_summary.csv")) %>% 
  clean_names() %>% 
...
test_summary <- as.data.frame(table(costs_data$test))

write_csv(test_summary, here("data", "2023_chemlab_test_summary.csv"))

```

```{r total_samples, include=FALSE}

# Total number of samples logged into Sample Master
total_sm <- length(unique(costs_data$sample_id))

# Total number of samples analysed by the Chemistry Lab
total_filt <- costs_data %>% 
...
write_csv(total_filt, here("data", "2023_chemlab_test_df.csv"))

phyto <- data %>% 
  filter(test == "Phytoplankton taxonomy")
  
phyto_count <- length(unique(phyto$sample_id))

zoop <- costs_data %>% 
  filter(test == "Zooplankton taxonomy")
  
zoop_count <- length(unique(zoop$sample_id))


total_anal <- length(unique(total_filt$sample_id))
```

```{r test_summary_plot, include = FALSE}

test_summary <- as.data.frame(table(total_filt$test))
...
```{r write_test_files, include = FALSE}

write_csv(test_summary, here("data", "2023_chemlab_test_summary.csv"))

save_plot(here("figures", "2023_chemlab_test_summary.pdf"), test_summary_plot)

```

```{r costs, echo=FALSE}

total_external_costs <- costs_data %>% 
  filter(customer_id !="Blanks") %>% 
  filter(customer_id !="Broadscale monitoring") %>% 
  filter(customer_id !="Diversion") %>% 
  filter(customer_id != "FAACTS") %>% 
  filter(customer_id !="LTER") %>% 
  filter(customer_id != "Proficiency testing") %>% 
  filter(customer_id != "SHRRIMP") %>% 
  filter(test != "Phytoplankton taxonomy") %>% 
  filter(test != "Zooplankton taxonomy")
...
customers <- (unique(total_external_costs$customer_id))

total_costs <- costs_data %>% 
  filter(test != "Phytoplankton taxonomy") %>% 
  filter(test != "Zooplankton taxonomy")

total_costs_count <- sum(total_costs$price, na.rm = TRUE)
...
```{r write_df, include = FALSE}

project_costs <- total_costs %>%
  group_by(customer_id) %>% 
  summarise(total = sum(price, na.rm = TRUE))
  
glimpse(project_costs)
...
write_csv(project_costs, here("data", "2023_chemlab_costs_summary.csv"))

save_plot(here("rmarkdown", "costs_summary_plot.pdf"), costs_summary_plot, base_height = 6)

save_plot(here("figures", "2023_chemlab_cost_summary.pdf"), costs_summary_plot, base_height = 6)
...
# 2023 IISD-ELA Analytical Service Lab Summary
Sonya Havens\
`r Sys.Date()`
...
The total number of samples processed by the IISD-ELA Analytical Service Lab (IISD-ASL) in 2023 was `r total_anal`. The following plot displays how many samples were analysed for each test in 2023.
...
The following plot provides the breakdown of the capital assets budget forecast and actual expenditures in the 2022-23 FY. All of the 2022-23 FY capital assets have been purchased and installed. The actual costs of the Autoclave and Centrifuge were slightly higher than forecasted. All other purchases were within the forecasted budget. 
...
The following plot breaks down the analytical costs by project. The Blanks, Broadscale monitoring, Diversion, FAACTS, LTER, and Proficiency testing, and SHRRIMP project costs are covered by core funding. The 2023 total cost recovery for the externally funded projects was $`r total_external_costs_count`.
